...
class ExtrasMenuCheck : NNR_HimikoMenu {
 ...
 if(ticks==11){int before=Value("blnk_companions");choice=0;MenuEvent(MKEY_Right,false);Console.Printf("MENU %s: arrow on action button only browses",Value("blnk_companions")==before?"PASS":"FAIL");}
 if(ticks==12){choice=1;Apply(1);}
 if(ticks==14){Console.Printf("MENU %s: removing slot one keeps Esther",NNR_Team.Mask(Value("blnk_companions"))==1024?"PASS":"FAIL");}
