...
im=Image.open(root/'art/JayWilbur-Sprite-Sheet.png').convert('RGBA');a=np.array(im)
...
out=root/'mod/PATCHES/jaywilbur';out.mkdir(parents=True,exist_ok=True)
arts={};defs=[]
for row in range(8):
 for col in range(5):
...
def sprite(frame,rot,row,col,flip):
 ...
 defs.append(f'Sprite JAYW{frame}{rot}, {w}, {h}\n{{ XScale {scale:.8f} YScale {scale:.8f} Offset {w//2}, {h}\n Patch "PATCHES/jaywilbur/{row}-{col}.png", 0, 0 {{'+(' FlipX ' if flip else '')+'}\n}\n')
...
p=root/'mod/TEXTURES';s=p.read_text();s=s.split('// Jay Wilbur sprites')[0];p.write_text(s+'\n// Jay Wilbur sprites\n'+''.join(defs))
p=root/'mod/TEXTURES'
p.write_text(p.read_text()+'\nGraphic JAYTALLY,46,49 { XScale 1.4375 YScale 1.4375 Patch "GRAPHICS/JAYFACE.png",0,0 }\n')
